...
export function EmailWindow({ isMaximized, onClose, onMinimize, onMaximize, onFocus, zIndex }: WindowProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 1. Tentar fazer o upload via Supabase primeiro se as chaves estiverem disponíveis
      try {
        ...
        if (supabase) {
          ...
          if (!uploadError) {
            ...
            if (data?.publicUrl) {
              ...
              proxiedUrl = `/api/proxy-storage?url=${encodeURIComponent(rawUrl.trim())}`;
              uploadedSuccessfully = true;
              console.log("Upload via Supabase concluído com sucesso.");
            }
          } else {
            console.warn("Supabase recusou ou falhou no upload. Iniciando fallback via servidor... Erro:", uploadError);
          }
        }
      } catch (supabaseErr: any) {
        console.warn("Supabase indisponível ou não configurado na máquina cliente. Iniciando upload alternativo via servidor...", supabaseErr);
      }

      // 2. Se o Supabase falhou ou não está configurado, faz o upload usando o serviço do servidor (/api/upload-anexo)
      if (!uploadedSuccessfully) {
        console.log("Realizando upload alternativo por meio de proxy servidor /api/upload-anexo...");
        const arrayBuffer = await file.arrayBuffer();
        const uploadRes = await fetch('/api/upload-anexo', {
          method: 'POST',
          headers: {
        ...
        if (!uploadRes.ok) {
          ...
          throw new Error(errorMsg || `Servidor respondeu com código de erro ${uploadRes.status}`);
        }

        const directUrl = await uploadRes.text();
        if (!directUrl || !directUrl.startsWith('http')) {
          throw new Error("Resposta do servidor de upload inválida");
        }

        proxiedUrl = `/api/proxy-storage?url=${encodeURIComponent(directUrl.trim())}`;
        uploadedSuccessfully = true;
        console.log("Upload alternativo via servidor concluído com sucesso.");
      }
